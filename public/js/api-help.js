var createFormatedOp = function(json, parent) {
	...
	$(invalidEmailTable).append($("<tr/>").append($("<th/>", {html: "Invalid Emails"})).append($("<th/>", {html: "Reason", colspan: 7}))).appendTo(parent);
	...
	if(json.invalidEmail.length > 0) {
		$.each(json.invalidEmail, function(i, e) {
			...
			$("<tr/>").append($("<td/>", {
				...
				html: e.reason.highRiskRecipientCheck && e.reason.highRiskRoleCheck ? "&#x2713;" : "&times;",
				style: "background: " + (e.reason.highRiskRecipientCheck && e.reason.highRiskRoleCheck ? "green" : "red"),
				title: "High Risk Recipient Check - " + (e.reason.highRiskRecipientCheck && e.reason.highRiskRoleCheck ? "Pass" : "Fail"),
				class: "reason-cell"
			})).append($("<td/>", {
